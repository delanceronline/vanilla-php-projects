...
var gIsEmailValid = false;
var gIsVerifyCodeValid = false;
...
$(document).ready(function()
{
	$('#email_button').click(function(){
		if($('#email').val() != '')
		{
			...
			$.get("./ajax_calls/ac_check_email.php", {email: $('#email').val(), rid: rn},
				  function(data){
					if(data == "1")
					{
						//alert('yes');
						$('#div_note_email_failed').hide();
						//$('#div_note_email_ok').show('slow');
						$('#div_note_email_ok').show();
						gIsEmailValid = true;
					}
					else
					{
						//$('#div_note_email_failed').show('slow');
						$('#div_note_email_failed').show();
						$('#div_note_email_ok').hide();
						gIsEmailValid = false;						
					}
			 });	
		}
	...
	$("#verify_button").click(function(){									 		
		...
		if(rrf != '' && rcf != '')
		{			
			$.get("./ajax_calls/ac_check_vc.php", { recaptcha_response_field: rrf, recaptcha_challenge_field: rcf },
				  function(data){
					if(data == "1")
					{
						$('#div_note_vc_failed').hide();
						//$('#div_note_vc_ok').show('slow');
						$('#div_note_vc_ok').show();
						gIsVerifyCodeValid = true;
					}
					else
					{
						//$('#div_note_vc_failed').show('slow');
						$('#div_note_vc_failed').show();
						$('#div_note_vc_ok').hide();
						gIsVerifyCodeValid = false;
...
function HandleFormSubmit()
{
	...
	*/
	...
	if($("#email").val() == '' || gIsEmailValid == false)
	{
		$('#error_email').dialog('open');
		return;	
	}

	if($("#recaptcha_response_field").val() == '' || gIsVerifyCodeValid == false)
	{
		$('#error_vc').dialog('open');
		return;	
	}	
	
	$("#tutor_register_form").submit();
}