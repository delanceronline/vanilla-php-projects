...
function checkTableName($shortTName, $type=false)
{
	...
	if ("campaigns" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
...
function GetLWWhere($field,$table="")
{
	...
	if(!$table) 
		$table = $strTableName;
	if($table=="campaigns" && $field=="status")
		return "language='".$_SESSION['language']."'";
	if($table=="clients" && $field=="industry")
		return "language='".$_SESSION['language']."'";
	if($table=="clients" && $field=="status")
		return "language='".$_SESSION['language']."'";
	return "";
}
...
{
	$arr=array();
	$strPerm = GetUserPermissions("campaigns");
...
// long binary data?
	...
	else if($format == FORMAT_FILE_IMAGE)
	{
		...
		$thumbnailed=false;
		$thumbprefix="";
		if($strTableName=="campaigns" && $field=="coupon")
		{
			$thumbnailed=true;
			$thumbprefix="";
		}
		if($strTableName=="campaigns" && $field=="pi")
		{
			$thumbnailed=true;
			$thumbprefix="";
		}
		if($strTableName=="campaigns" && $field=="ti")
		{
			$thumbnailed=true;
			$thumbprefix="";
		}
		if($strTableName=="clients" && $field=="logo")
		{
			$thumbnailed=true;
			$thumbprefix="";
		}
...
function GetUserPermissionsStatic($table="")
{
	...
	if($table=="campaigns" && $sUserGroup=="0")
	{
				return "AEDSPI";
	}
	if($table=="campaigns" && $sUserGroup=="2")
	{
				return "AEDSPI";
	}
	if($table=="campaigns" && $sUserGroup=="1")
	{
				return "AEDSPI";
	}
//	default permissions	
	...
	if($table=="clients" && $sUserGroup=="0")
	{
				return "ESI";
	}
	if($table=="clients" && $sUserGroup=="2")
	{
				return "AEDSPI";
	}
	if($table=="clients" && $sUserGroup=="1")
	{
				return "AEDSPI";
	}
//	default permissions	
