...
$templatefile = "campaigns_edit.htm";
	...
	if($condition)
	{
		...
		if(FieldSubmitted("coupon_".$id))
		{
			...
			if(substr($type,0,4)=="file")
				$value = prepare_file($value,"coupon",$type,$fileNameForPrepareFunc ,$id);
...
// file deletion, read filename from the database
					...
					$fileNameForPrepareFunc = $dataold["coupon"];
				}
				$value = prepare_upload("coupon",$type,$fileNameForPrepareFunc,$value,"" ,$id,$pageObject);
			}
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "coupon"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["coupon"] = $value;
		
			}
	
		}
//	processing coupon - end
//	processing cn_offset - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "cn_offset"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["cn_offset"] = $value;
		
			}
	
		}
//	processing cn_offset - end
//	processing cn_prefix - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "cn_prefix"=="password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["cn_prefix"] = $value;
		
			}
	
		}
//	processing cn_prefix - end
//	processing cc - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	if($condition)
	{
		...
		if(FieldSubmitted("pi_".$id))
		{
			$fileNameForPrepareFunc = postvalue("filename_pi_".$id);
				...
				$value = $fileNameForPrepareFunc;
			if(substr($type,0,4)=="file")
				$value = prepare_file($value,"pi",$type,$fileNameForPrepareFunc ,$id);
			else if(substr($type,0,6)=="upload")
			{
				if($type=="upload1")
				{
// file deletion, read filename from the database
					...
					$fileNameForPrepareFunc = $dataold["pi"];
				}
				$value = prepare_upload("pi",$type,$fileNameForPrepareFunc,$value,"" ,$id,$pageObject);
			}
		}
		else
			$value = false;
	...
	if($condition)
	{
		...
		if(FieldSubmitted("ti_".$id))
		{
			$fileNameForPrepareFunc = postvalue("filename_ti_".$id);
				...
				$value = $fileNameForPrepareFunc;
			if(substr($type,0,4)=="file")
				$value = prepare_file($value,"ti",$type,$fileNameForPrepareFunc ,$id);
			else if(substr($type,0,6)=="upload")
			{
				if($type=="upload1")
				{
// file deletion, read filename from the database
					...
					$fileNameForPrepareFunc = $dataold["ti"];
				}
				$value = prepare_upload("ti",$type,$fileNameForPrepareFunc,$value,"" ,$id,$pageObject);
			}
		}
		else
			$value = false;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: campaigns_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_campaigns->Copy();
...
if(!$data)
{
	...
	{
		header("Location: campaigns_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["cn_offset"] = $evalues["cn_offset"];
	$data["cn_prefix"] = $evalues["cn_prefix"];
	$data["cc"] = $evalues["cc"];
	$data["pt"] = $evalues["pt"];
	$data["pd"] = $evalues["pd"];
