    ...
    if ($db_selected) {
        ...
        if (isset($_POST['id']) && $_POST['id'] != null) {
            ...
            if ($result) {
                ...
                if ($check) {
                    $result = mysql_query("UPDATE `product_cat` SET `title`='$title',`sort`='$sort',`image`='$image' WHERE id = '$id'");
                }
            }
        } else {
            $result = mysql_query("INSERT INTO `product_cat`(`title`, `sort`, `image`) values ('$title', '$sort', '$image')");
