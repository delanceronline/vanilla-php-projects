    ...
    if ($db_selected) {
        ...
        if (isset($_POST['id']) && $_POST['id'] != null) {
            ...
            if ($result) {
                ...
                if ($check) {
                    $result = mysql_query("UPDATE `product` SET `title`='$title',`sort`='$sort',`show`='$show',`image`='$image',`category_id`='$category',`brand_id`='$brand',`model`='$model',`new_product`='$new_product',`function`='$function',`description`='$description',`size`='$size',`link`='$dlink' WHERE id = '$id'");
                }
            }
        } else {
            $result = mysql_query("INSERT INTO `product`(`title`, `sort`, `show`, `image`, `category_id`, `brand_id`, `model`, `new_product`, `function`, `description`, `size`, `link`) values ('$title', '$sort', '$show', '$image', '$category', '$brand', '$model', '$new_product', '$function', '$description', '$size', '$dlink')");
