<?php 
		...
		$data = array();
		if($_GET['type']!=0)
		$db_connector->GetRows($data, "WHERE i_group={$_GET['type']}", "client_shop");
		else
		$db_connector->GetRows($data, "WHERE i_group!=0 ORDER BY i_group", "client_shop");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 ...
 <td width="11%"><input type="submit" name="button3" id="button3" value="更改" /><input name="pid" id="pid" type="hidden" value="<?php echo $data[$i]['ID']; ?>"><input name="ctype" id="ctype" type="hidden" value="<?php if (isset($_GET['type'])) echo $_GET['type']; else echo "0"; ?>"></td> </form>
       <form id="form1" name="form1" method="post" action="delete_client_shop.php">
