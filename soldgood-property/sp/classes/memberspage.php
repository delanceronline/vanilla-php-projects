...
{	
	...
	{	
		...
		$this->xt->assign("addgroup_attrs","onclick=\"\$('#addarea').show();\$('#groupname')[0].focus(); \$('#groupname').val(makename('".mlang_message("AA_GROUP_NEW")."'));\$('#gmessage').html(TEXT_AA_ADD_NEW_GROUP);renameidx=-1;\"");
	...
	{							
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			$row["user"]=$data["login"];
			$usercheckbox_attrs.=" name=\"user_".htmlspecialchars($data["login"])."\"";
