...
function checkTableName($shortTName, $type=false)
{
	...
	if ("supplies" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("supply_modes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("supply_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	...
	if ("zones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("heights" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("identity" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("property_states" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	return false;
...
function GetLWWhere($field,$table="")
{
	...
	if(!$table) 
		$table = $strTableName;
	if($table=="supplies" && $field=="zone_id")
		return "language_type='".$_SESSION['language']."'";
	if($table=="supplies" && $field=="district_id")
		return "language_type='".$_SESSION['language']."'";
	if($table=="supplies" && $field=="supply_mode_id")
		return "language_type='".$_SESSION['language']."'";
	if($table=="supplies" && $field=="support_type_id")
		return "language_type='".$_SESSION['language']."'";
	if($table=="supplies" && $field=="height_id")
		return "language_type='".$_SESSION['language']."'";
	if($table=="supplies" && $field=="ps_id")
		return "language_type='".$_SESSION['language']."'";
	if($table=="users" && $field=="identity_id")
		return "language_type='".$_SESSION['language']."'";
	return "";
}
...
function GetTablesList($pdfMode=false)
{
	...
	$strPerm = GetUserPermissions("supplies");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="supplies";
	}
	$strPerm = GetUserPermissions("supply_modes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="supply_modes";
	}
	$strPerm = GetUserPermissions("supply_types");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="supply_types";
	}
	...
	$strPerm = GetUserPermissions("heights");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="heights";
	}
	$strPerm = GetUserPermissions("identity");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="identity";
	}
	$strPerm = GetUserPermissions("property_states");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="property_states";
	}
...
function GetTableURL($table="")
{
	...
	if("supplies"==$table) 
		return "supplies";
	if("supply_modes"==$table) 
		return "supply_modes";
	if("supply_types"==$table) 
		return "supply_types";
	if("users"==$table) 
		return "users";
	if("zones"==$table) 
...
{	
	...
	if("districts"==$shortTName) 
		return "districts";
	if("supplies"==$shortTName) 
		return "supplies";
	if("supply_modes"==$shortTName) 
		return "supply_modes";
	if("supply_types"==$shortTName) 
		return "supply_types";
	if("users"==$shortTName) 
		return "users";
	if("zones"==$shortTName) 
		return "zones";
	if("heights"==$shortTName) 
		return "heights";
	if("identity"==$shortTName) 
		return "identity";
	if("property_states"==$shortTName) 
		return "property_states";
}
...
// long binary data?
	...
	else if($format == FORMAT_FILE_IMAGE)
	{
		...
		$thumbnailed=false;
		$thumbprefix="";
		if($strTableName=="supplies" && $field=="icon")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic1")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic2")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic3")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic4")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic5")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic6")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic7")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="supplies" && $field=="pic8")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="users" && $field=="login")
		{
			$thumbnailed=true;
			$thumbprefix="";
		}
		if($strTableName=="users" && $field=="icon")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
...
//	filename edit
			...
			if ( $edit==MODE_INLINE_EDIT ) {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked>'.mlang_message("KEEP");
			} else {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked>'.mlang_message("KEEP");
			}
			...
			if((strlen($value) || $edit==MODE_INLINE_EDIT) && !IsRequired($field))
			{
				$strtype.='<input id="'.$ctype.'_delete" type="Radio" name="'.$ctype.'" value="upload1">'.mlang_message("DELETE");
			}
			$strtype.='<input id="'.$ctype.'_update" type="Radio" name="'.$ctype.'" value="upload2">'.mlang_message("UPDATE");
		}
		else
		{
//	if Adding record		
